...
def _render_json(app, form, request, include_user=True, include_auth_token=False, include_token_expiry=False):
    ...
    if has_errors:
        ...
        response['user']['verify'] = str(form.user.first_name)        
        response['mydata'] = request.json
...
def login_with_token(request, app):
    ...
    if request.json:
        return _render_json(app, form, request, include_auth_token=True, include_token_expiry=True)